...
## 2025-01-28 - Performance Anti-Pattern: Full DOM Scans in Observers
**Learning:** Using `document.querySelectorAll` inside a `MutationObserver` callback causes O(N) scans on every mutation. For large documents, this is catastrophic.
**Action:** Always iterate over `mutation.addedNodes` to process only new elements.
