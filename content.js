...
function observeNotebookLM() {
    ...
    // Initial scan to catch notes present before observation starts
    const existingNotes = document.querySelectorAll(noteSelector);
    existingNotes.forEach(processNote);
    console.log(`NotebookLM Chat: Initial scan found ${existingNotes.length} notes.`);

    const observer = new MutationObserver((mutationsList, observer) => {
        // Optimization: Iterate only over added nodes instead of rescanning the entire DOM
        // This reduces complexity from O(N) to O(M) where M is the number of added nodes.
            ...
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) { // ELEMENT_NODE
                        ...
                        // Also check for nested notes inside the added node (e.g. a container was added)
